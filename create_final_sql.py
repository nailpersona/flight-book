# -*- coding: utf-8 -*-
# Simple parser that inserts all sections in order_num order
# This ensures parents exist before their children are referenced
...
def parse_insert_line(line):
    ...
    # Use regex to split by comma, respecting $$ delimiters
    # Pattern: split by comma that's NOT within $$...$$
    parts = re.split(r',(?!(?:[^,]*\$\$[^,]*\$\$)*[^,]*$)', values_str)

    # Now extract fields
    # We should have at least: doc_id, parent_id, title, content or NULL, order_num
    if len(parts) >= 5:
        doc_id = parts[0].strip()
        parent_id = parts[1].strip()
        title = parts[2].strip("'\"")
        # Everything from title to end is content (including $$...$$)
        content = ','.join(parts[2:-1]) if len(parts) > 3 else None
        order_num = parts[-1].strip()
        return [doc_id, parent_id, title, content, order_num]
    return None
...
for match in matches:
    ...
    if parts:
        doc_id, parent_id, title, content, order_num = parts
        all_sections.append({
            'doc_id': doc_id,
            'parent_id': parent_id,
            'title': title,
...
# Group into batches of 50 for better readability
batch_size = 50
